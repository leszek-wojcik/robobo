...
# FreeRTOS
FREERTOS_LIB_DIR = src/FreeRTOS/
FREERTOS_LIB_C_FILES = $(call cfilesfromdir,$(FREERTOS_LIB_DIR))  
FREERTOS_LIB_CPP_FILES = $(call cppfilesfromdir,$(FREERTOS_LIB_DIR))   
FREERTOS_LIB_OBJ_DIR = $(FREERTOS_LIB_DIR)$(OBJ_DIR)
FREERTOS_C_OBJ_FILES =   $(call objectfilesfromlist, $(FREERTOS_LIB_OBJ_DIR), $(FREERTOS_LIB_C_FILES))
FREERTOS_CPP_OBJ_FILES = $(call objectfilesfromlist, $(FREERTOS_LIB_OBJ_DIR), $(FREERTOS_LIB_CPP_FILES))

# ALL TARGET
ALL_C_OBJ_FILES =$(ARDUINO_LIB_C_OBJ_FILES) $(AVR_LIB_C_OBJ_FILES) $(ROBOBO_LIB_C_OBJ_FILES) $(FREERTOS_C_OBJ_FILES)
ALL_CPP_OBJ_FILES = $(ARDUINO_LIB_CPP_OBJ_FILES) $(ROBOBO_LIB_CPP_OBJ_FILES) $(FREERTOS_CPP_OBJ_FILES)
...
OBJ_DIRS = $(ROBOBO_UT_OBJ_DIR) $(ROBOBO_LIB_OBJ_DIR) $(AVR_LIB_OBJ_DIR) $(ARDUINO_LIB_OBJ_DIR) $(FREERTOS_LIB_OBJ_DIR)
...
clean :
	...
	@rm -f -r $(FREERTOS_LIB_DIR)$(OBJ_DIR)
...
$(FREERTOS_LIB_DIR)$(OBJ_DIR)%.o : $(FREERTOS_LIB_DIR)%.c
	@echo "Compiling $@"
	@$(CC) $(CC_FLAGS) $(INCLUDE_FILES) -o $@ $<

$(FREERTOS_LIB_DIR)$(OBJ_DIR)%.o : $(FREERTOS_LIB_DIR)%.cpp
	@echo "Compiling $@"
